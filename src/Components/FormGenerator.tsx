    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      {schema.fields.map((field) => (
        <div key={field.id} className="flex flex-col">
            ...
            <input
              ...
              {...register(field.id, {
                ...
                pattern: field.validation?.pattern
                  ? { value: new RegExp(field.validation.pattern), message: field.validation.message || 'Invalid format' }
                  : undefined,
              })}
              className={`border p-2 rounded ${errors[field.id] ? 'border-red-500' : 'border-gray-300'}`}
            />
          ) : null}
          ...
          {errors[field.id] && (
            <p className="text-red-500 text-sm mt-1">{(errors[field.id] as any).message}</p>
          )}
        </div>
      ))}
